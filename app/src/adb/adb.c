 ...
 */
#define SC_ADB_COMMAND(...)                        \
    {                                              \
        sc_adb_get_executable(), __VA_ARGS__, NULL \
    }
...
bool sc_adb_start_server(struct sc_intr *intr, unsigned flags)
{
...
bool sc_adb_kill_server(struct sc_intr *intr, unsigned flags)
{
...
bool sc_adb_forward_remove(struct sc_intr *intr, const char *serial,
...
bool sc_adb_reverse_remove(struct sc_intr *intr, const char *serial,
                           const char *device_socket_name, unsigned flags)
{
    char remote[108 + 14 + 1]; // localabstract:NAME
    snprintf(remote, sizeof(remote), "localabstract:%s", device_socket_name);
...
bool sc_adb_push(struct sc_intr *intr, const char *serial, const char *local,
...
bool sc_adb_install(struct sc_intr *intr, const char *serial, const char *local,
                    unsigned flags)
{
#ifdef __WINDOWS__
...
bool sc_adb_select_device(struct sc_intr *intr,
                          const struct sc_adb_device_selector *selector,
...
bool sc_adb_is_serial_tcpip(const char *serial)
{
